...
import { emojiGenshinList } from './emoji_genshin';

/**
 * 处理文本中的表情代码，将其转换为HTML图片标签
 * @param content 包含表情代码的原始文本
 * @param emojiType 表情类型：0-普通表情，1-原神表情
 * @returns 处理后的HTML内容
 */
export function processEmoji(content: string, emojiType: number = 0): string {
  ...
  try {
    ...
    for (let i = matches.length - 1; i >= 0; i--) {
      ...
      // 根据表情类型选择不同的表情集
      const currentEmojiList = emojiType === 0 ? emojiList : emojiGenshinList;
      
      // 如果表情存在于表情集中，则替换为对应的HTML
...
export function cleanupContent(content: string): string {
  ...
  try {
    // 先尝试处理常规表情
    let cleanedContent = processEmoji(content, 0);
    
    // 再尝试处理原神表情
    cleanedContent = processEmoji(cleanedContent, 1);
